...
def read_race_file(file_path):
    ...
    with open(file_path, "r") as file:
        ...
        craw_constant = car["crawl_constant_m/s"]
...
def max_corner_speed(tyre_degradation,  radius, base_friction_coefficient, weather_multiplier, crawl_constant):
    tyre_friction = total_tyre_friction(tyre_degradation,  base_friction_coefficient, weather_multiplier)
...
def main():
    ...
    current_lap = 0
    output_dict = dict()
    output_dict = decision_at_lap(race["laps"], output_dict, tyres, 0, 0 )
    with open("output.txt","w") as file:
        json.dump(output_dict, file)


def decision_at_lap(laps, output_dict,  tyres, time,  speed):
    outputs = []
    # consider refuel
    # combinations of refuel no refuel , and change in tyre
    # enter pit or no

    for new_tyre  in tyres.values():
        output_dict = deepcopy(output_dict)
        tyre_output , tyre_time, tyre_speed = dict(),0, 0
        output_dict["laps"] = []

        for lap in range(laps):
            output_dict["initial_tyre_id"] = new_tyre["id"]
            laps_dict = dict()
            laps_dict ["lap"] = lap
            output_dict["laps"].append(laps_dict)
            
        # reduce number of tyres
        # calc lap 
            tyre_output, tyre_time, tyre_speed = calc_lap(lap, tyre_output, new_tyre, time,  tyre_speed)
        ...
        # calculate
        outputs.append( (tyre_output, tyre_time  ))
   ...
   # get if not  
    ...
    for output in outputs:
        ...
        score = base_score_level_1(race["time_reference_s"], output[1])
        if score >= max_score:
            max_output = output[0] 
            max_score = score
...
def get_time_straight(initial, final, accel):
    return (final - initial) / accel

def calc_lap(lap, output_dict,tyre ,time, speed):
    ...
    for i in range(len(segments)):
        ...
        segment_dict["type"] =segment["type"] 
        length = segment["length"]
        seg_type = segment["type"]
        accel = car["accel_m/se2"]
        break_acc = car["brake_m/se2"]
        if (segment["type"] == "straight"):
            ...
            if i != len(segments)-1:
                ...
                weather = weathers[0]
                max_corner = max_corner_speed(tyre["dry_degradation"],  corner["radius"], k_base ,weather["acceleration_multiplier"], crawl_constant)

                diff_speed = max_corner -speed
                break_dist = diff_speed/2 * (diff_speed/accel)
                delta_t = (abs(diff_speed)/break_acc) 
                if diff_speed < 0 :
                    break_dist = abs(diff_speed)/2 *  delta_t 
                
                break_point = max(0, length-break_dist)

                time += (break_point/speed if speed else  0 )+  delta_t
                if speed ==0:
                    time += break_point/max_corner
                speed = max_corner 
                segment_dict["target_m/s"] = max_corner
                segment_dict["brake_start_m_before_next/s"] = break_point 
